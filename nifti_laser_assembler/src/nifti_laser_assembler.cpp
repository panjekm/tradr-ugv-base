...
protected:
	...
	void append_scan(sensor_msgs::PointCloud2 scan);
 ...
 */
void NiftiLaserAssembler::append_scan(sensor_msgs::PointCloud2 scan)
{
	...
	} else
	{
		...
		if (tf_listener.waitForTransform(world_frame, scan.header.frame_id,
			...
			ros::Duration offsetTime = tmp_point_cloud.header.stamp - point_cloud.header.stamp;
//			sensor_msgs::PointCloud2::_fields_type::iterator
//			scan.
//
//      ROS_INFO_STREAM("[NLA] Append_scan original time: " << point_cloud.header.stamp << " new scan time: " << tmp_point_cloud.header.stamp <<
//                      " diff: " << tmp_point_cloud.header.stamp - point_cloud.header.stamp << " floated: " << float((tmp_point_cloud.header.stamp - point_cloud.header.stamp).toSec()));
			for(sensor_msgs::PointCloud2::_fields_type::const_iterator it = tmp_point_cloud.fields.begin(); it != tmp_point_cloud.fields.end(); ++it){
			  if (it->name == "stamps") {

			    uint8_t* dataPtr(&tmp_point_cloud.data[0]);
	        for (size_t x(0); x < tmp_point_cloud.width; ++x)
	        {
	          uint8_t* fPtr(dataPtr + it->offset);

//	          ROS_INFO_STREAM("[NLA] Append_scan Value of *fPtr is " << float(*reinterpret_cast<const float*>(fPtr)) << " at x= " << x);
	          float newStamp = float(*reinterpret_cast<const float*>(fPtr)) + float(offsetTime.toSec());

//            ROS_INFO_STREAM("[NLA] Append_scan wanting it to be " << newStamp);

            memcpy(fPtr, reinterpret_cast<const uint8_t*>(&newStamp), 4);
//	          ROS_INFO_STREAM("[NLA] Append_scan Value of *fPtr is now " << float(*reinterpret_cast<const float*>(fPtr)));

	          dataPtr += tmp_point_cloud.point_step;
	        }
